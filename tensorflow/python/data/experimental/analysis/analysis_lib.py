...
class PlumberPerformanceModel(object):
  ...
  def __init__(self, filename, use_CPU_time=False):
    ...
    self.root = self.get_complete_tree(use_CPU_time)  # Interface for in-memory tree
  ...
  def get_ratios(self):
    rates_dict = {}
    for s in self.plumber_data.stats:
      rates_dict[s.name] = s.element_ratio / s.count
    return rates_dict
  ...
  def get_complete_tree(self, use_CPU_time=False):
    ...
    all_data = [rates, cpu_rates, abs_rates, read_rates, ratios, counts,
                size_bytes]
...
class PlumberRecommendation:
  ...
  def current_rate(self):
    return self.plumber_model.root.slowest_node().expected_observed_rate()

  def fix_rate(self):
    slow_node = self.plumber_model.root.slowest_node()
    theoretical_rate = slow_node.max_possible_rate()
    second_node = self.plumber_model.root.slowest_node(ignore_set=[slow_node])
    fix_rate = second_node.cum_rate()
    return min(fix_rate, theoretical_rate)
...
class TreeNode:
  ...
  def to_graphviz_str(self):
    ...
    for i in self.__iter__():
      ...
      if (i.pipeline_time_percentage() is not None and
        ...
        pipe_perc = "{:.2%}".format(i.pipeline_time_percentage())
      ...
      if i.f:
        ...
        for j in i.f:
          ...
          if edge is None:
            ...
            graphviz_str += "        {} -> {} [ label = \"{:.2e}\" ];\n".format(
                src, dst, edge)
          if j.is_src():
            shape_info_str += "    {} [shape=Mdiamond];\n".format(src)
          if j.is_stale:
            shape_info_str += "    {} [color=gray];\n".format(src)
          if (j.pipeline_time_percentage() is not None and
              j.p_busy_pipeline() is not None and
              j.rate_slowdown() is not None):
            pipe_perc = "{:.2%}".format(j.pipeline_time_percentage())
            ...
            if j.is_src():
              bw = j.read_rate
              if bw is not None:
                bw = "{:.2e} MB/s".format(bw / 1e6)
              label = "{dst}(\\BW: {bw})".format(dst=src, bw=bw)
            else:
  ...
  def slowest_node(self, return_percent_time=False, ignore_stale_nodes=True, ignore_set=None):
    ...
    if ignore_stale_nodes:
      all_nodes = list(filter(lambda x: not x.is_stale and x not in ignore_set, all_nodes))
